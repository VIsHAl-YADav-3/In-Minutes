...
const Navbar = ({ onOpenChat }) => {
  ...
  // Supports both old and new customer roles
  const isCustomer =
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-ink-900/5 bg-white/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-4">
        ...
        <Link
          to="/"
          className="flex shrink-0 items-center gap-1.5 font-display text-xl font-extrabold text-brand-500"
        >
          <span className="rounded-lg bg-brand-500 px-2 py-1 text-sm text-white">
            IM
          </span>
            ...
            <div className="flex flex-col divide-y divide-ink-900/5">
              ...
              {isSeller && (
                <Link
                  to="/seller"
                  onClick={() => setMenuOpen(false)}
                  className="py-2.5 text-sm font-medium text-brand-600"
                >
                  Seller Dashboard
                </Link>
              )}
